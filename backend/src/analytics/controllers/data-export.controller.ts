...
export class DataExportController {
  ...
  @ApiOperation({
    ...
    description:
      'Exports historical match data with features for machine learning model training. Admin only.',
  })
  @ApiResponse({
    status: 200,
  ...
  async exportTrainingData(
    ...
    const minMatchesPerTeam = params.minMatchesPerTeam ?? 5;
    ...
    if (minMatchesPerTeam < 1 || minMatchesPerTeam > 50) {
      throw new BadRequestException(
        'minMatchesPerTeam must be between 1 and 50',
      );
    }
  ...
  @ApiOperation({
    ...
    description:
      'Exports historical match data as a downloadable CSV file for ML training. Admin only.',
  })
  @ApiResponse({
    status: 200,
    ...
    headers: {
      ...
      'Content-Disposition': {
        description: 'attachment; filename="training-data.csv"',
      },
    },
  })
  ...
  @Header(
    'Content-Disposition',
    'attachment; filename="footdash-training-data.csv"',
  )
  async exportTrainingDataCSV(
    @Query('seasons', new ParseArrayPipe({ items: String, separator: ',' }))
    seasons?: string[],
    @Query('leagues', new ParseArrayPipe({ items: Number, separator: ',' }))
    leagues?: number[],
    ...
    const minMatches = minMatchesPerTeam ?? 5;
    const exportParams: TrainingDataExportParams = {
    ...
    // Export data
    const result =
      await this.dataExportService.exportTrainingData(exportParams);
  ...
  @ApiOperation({
    ...
    description:
      'Returns a sample of training data structure for development purposes. Admin only.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getSampleTrainingData() {
    ...
    return {
      ...
      feature_descriptions: {
        ...
        is_home:
          'Whether team is playing at home (always true for home team perspective)',
  ...
  @ApiOperation({
    ...
    description:
      'Returns statistics about available data for export. Admin only.',
  })
  @ApiResponse({
    status: 200,
