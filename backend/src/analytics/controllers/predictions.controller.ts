...
export class PredictionsController {
  ...
  @ApiResponse({ status: 200, description: 'Prediction retrieved successfully', type: PredictionResultDto })
  @ApiResponse({ status: 404, description: 'Match not found' })
  async getMatchPrediction(
    @Param('matchId', ParseIntPipe) matchId: number,
    @Query('force') forceRecalculate?: boolean,
  ) {
    return await this.predictionService.getPrediction(matchId, forceRecalculate === true);
  }

  @Post('match/:matchId/predict')
  @ApiOperation({ summary: 'Generate new prediction for a match' })
  @ApiResponse({ status: 201, description: 'Prediction generated successfully', type: PredictionResultDto })
  @ApiResponse({ status: 404, description: 'Match not found' })
  ...
  @ApiResponse({ status: 200, description: 'Predictions retrieved successfully', type: [PredictionResultDto] })
