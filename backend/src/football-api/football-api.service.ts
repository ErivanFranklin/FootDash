...
export class FootballApiService implements FootballApiAdapter {
  ...
  async getTeamInfo(teamId: number) {
    ...
    await this.cache.set(
      cacheKey,
      result,
      FootballApiCacheService.TTL.TEAM_INFO,
    );
    return result;
  }
  ...
  async getTeamStats(params: TeamStatsParams) {
    ...
    await this.cache.set(
      cacheKey,
      result,
      FootballApiCacheService.TTL.TEAM_STATS,
    );
    return result;
  }
  ...
  async getTeamFixtures(params: TeamFixturesParams) {
    ...
    const hasPlanError =
      raw?.errors && typeof raw.errors === 'object' && 'plan' in raw.errors;
  ...
  async getFixtureLineups(fixtureId: number) {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to fetch lineups for fixture ${fixtureId}: ${(error as Error).message}`,
      );
      return [];
    }
  }
  ...
  async getLeagueStandings(leagueId: number, season: number): Promise<any[]> {
    ...
    try {
      const data = await this.makeRequest<ApiResponse<any>>('standings', {
        league: leagueId,
        season,
      });
      const standings =
        (data as any).response?.[0]?.league?.standings?.[0] ?? [];
      await this.cache.set(cacheKey, standings, 3600); // 1h
      return standings;
    } catch (error) {
      this.logger.warn(
        `Failed to fetch standings for league ${leagueId}: ${(error as Error).message}`,
      );
  ...
  async getLeagueFixtures(
    ...
    try {
      ...
      this.logger.warn(
        `Failed to fetch league fixtures: ${(error as Error).message}`,
      );
      return this.getMockLeagueFixtures(leagueId);
    }
  }

  private getMockLeagues(country?: string): any[] {
    void country;
    return [
      {
      ...
      {
        league: { id: 140, name: 'La Liga', type: 'League', logo: null },
        country: { name: 'Spain', code: 'ES', flag: null },
      },
      {
        league: { id: 135, name: 'Serie A', type: 'League', logo: null },
        country: { name: 'Italy', code: 'IT', flag: null },
      },
      {
        league: { id: 78, name: 'Bundesliga', type: 'League', logo: null },
        country: { name: 'Germany', code: 'DE', flag: null },
      },
      {
        league: { id: 61, name: 'Ligue 1', type: 'League', logo: null },
      ...
      {
        league: { id: 3, name: 'Europa League', type: 'Cup', logo: null },
        country: { name: 'World', code: null, flag: null },
      },
    ];
  }
  ...
  private getMockStandings(leagueId: number): any[] {
    ...
    return teams.map((name, idx) => ({
      ...
      all: {
        ...
        draw: idx,
        lose: 5 - (4 - idx),
        goals: { for: 25 - idx * 2, against: 10 + idx * 2 },
      },
    }));
  }
  ...
  private getMockLeagueFixtures(leagueId: number): any[] {
    return Array.from({ length: 5 }, (_, i) => ({
      ...
      teams: {
        home: { id: leagueId * 10 + i, name: `Home ${i + 1}` },
        away: { id: leagueId * 10 + i + 5, name: `Away ${i + 1}` },
      },
